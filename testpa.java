...
import javax.swing.*;
import java.sql.*;
class testpa extends JFrame implements ActionListener  
{  
    JLabel l;  
    ...
    {  
        ...
        l=new JLabel();  
        add(l);  
        bg=new ButtonGroup();  /* Grouping of Radiobuttons.Only */
        for(int i=0;i<5;i++)  
        {  
    ...
    public void actionPerformed(ActionEvent e)  /* Action Listener keyword - actionPerformed */
    {  
        if(e.getSource()==b1)  
        {  
            if(check())  
        ...
        {  
...

            {
                ...
                Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/swetha","root","Siva@123");

                Statement stmt = con.createStatement();
                String sql="insert into siva values(?)";
    ...
    {  
        if(current==0)  
            return(jb[1].isSelected());  /* isSelected - method name to select */
        if(current==1)  
    ...
    {  
        new testpa("Online Test Of Java");  
    }  
}
